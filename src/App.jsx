...
// ─── BOUTON 3D FLOTTANT ───────────────────────────────────────────────────────
function Btn({children,onClick,color="#6366f1",size="md",full=false,danger=false,disabled=false}){
  const[h,setH]=useState(false);const[p,setP]=useState(false);
  const c=danger?"#ef4444":color;const rgb=h2r(c);
  const pad=size==="sm"?"7px 13px":size==="lg"?"14px 26px":"10px 18px";
  ...
  return(
    <button
      ...
      style={{
        ...
        width:full?"100%":"auto",
        background:disabled?`rgba(${rgb},0.05)`:`rgba(${rgb},${h?0.30:0.14})`,
        backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",
        border:`1.5px solid rgba(${rgb},${disabled?0.08:h?0.6:0.28})`,
        borderRadius:13,
        boxShadow:disabled?"none":p
          ?`0 1px 3px rgba(${rgb},0.1),inset 0 1px 0 rgba(255,255,255,0.05)`
          :h
            ?`0 12px 28px rgba(${rgb},0.35),0 4px 8px rgba(${rgb},0.2),inset 0 1px 0 rgba(255,255,255,0.2),0 1px 0 rgba(${rgb},0.5)`
            :`0 6px 18px rgba(${rgb},0.22),0 2px 5px rgba(${rgb},0.12),inset 0 1px 0 rgba(255,255,255,0.14),0 1px 0 rgba(${rgb},0.38)`,
        transform:disabled?"none":p?"scale(0.97) translateY(2px)":h?"translateY(-3px)":"translateY(0)",
  ...
  return(
    <div style={S.app}>
      ...
      {modal==="event"&&selDay&&<div style={S.modal} onClick={e=>e.target===e.currentTarget&&setModal(null)}>
        <div style={S.mCard}>
          <div style={{fontWeight:900,fontSize:16,marginBottom:11,color:T.text}}>+ {L.add} · {selDay} {MOIS[month]}</div>
          <div style={S.disc}>{L.disc}</div>
          <div style={{marginBottom:10}}><div style={S.inpLbl}>Type</div><div style={{...S.row,gap:5,marginTop:4}}>{EVT_IDS.map((id,i)=><Pill key={id} active={newEvt.type===id} color={EVT_COLORS[i]} onClick={()=>setNewEvt(e=>({...e,type:id}))}>{L.evtTypes[i]}</Pill>)}</div></div>
          <div style={{marginBottom:10}}><div style={S.inpLbl}>Titre *</div><input style={S.inp} value={newEvt.titre} onChange={e=>setNewEvt(v=>({...v,titre:e.target.value}))} placeholder="Ex: Pédiatre, match de foot…" autoFocus/></div>
          <div style={{marginBottom:10}}><div style={S.inpLbl}>Heure</div><input type="time" style={S.inp} value={newEvt.heure} onChange={e=>setNewEvt(v=>({...v,heure:e.target.value}))}/></div>
          <div style={{marginBottom:16}}><div style={S.inpLbl}>Visibilité</div><div style={S.row}><Pill active={newEvt.shared} color={colorA} onClick={()=>setNewEvt(v=>({...v,shared:true}))}>{L.shared}</Pill><Pill active={!newEvt.shared} color="#6b7280" onClick={()=>setNewEvt(v=>({...v,shared:false}))}>{L.prive}</Pill></div></div>
          <div style={{display:"flex",gap:8}}><Btn color={colorA} size="lg" onClick={addEvent}>{L.ajouter}</Btn><Btn color="#6b7280" size="lg" onClick={()=>setModal(null)}>{L.annuler}</Btn></div>
        </div>
      </div>}

      {/* Modal note */}
      {modal==="note"&&selDay&&<div style={S.modal} onClick={e=>e.target===e.currentTarget&&setModal(null)}>
        <div style={S.mCard}>
